import React, { useState, useEffect, useMemo } from 'react';
import BoardColumn from '../components/BoardColumn';
import { getIssues } from '../services/issueApi';
import { Loader2, AlertTriangle, RefreshCcw } from 'lucide-react';

// ── Thứ tự và nhãn cột ────────────────────────────────────────
const COLUMN_STATUSES = ['TODO', 'IN_PROGRESS', 'TEST', 'DONE'];

// ── Component ──────────────────────────────────────────────────
const Board = () => {
  const [issues,  setIssues]  = useState([]);
  const [loading, setLoading] = useState(true);
  const [error,   setError]   = useState('');

  // Fetch issues từ backend
  const fetchIssues = async () => {
    setLoading(true);
    setError('');
    try {
      const res = await getIssues();
      setIssues(res.data ?? []);
    } catch (err) {
      setError(err.message || 'Không thể tải danh sách Issue. Kiểm tra backend đang chạy.');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchIssues();
  }, []);

  // Tách mảng tổng → { TODO: [], IN_PROGRESS: [], TEST: [], DONE: [] }
  const grouped = useMemo(() => {
    const map = { TODO: [], IN_PROGRESS: [], TEST: [], DONE: [] };
    for (const issue of issues) {
      if (map[issue.status] !== undefined) {
        map[issue.status].push(issue);
      }
    }
    return map;
  }, [issues]);

  // ── Loading state ────────────────────────────────────────────
  if (loading) {
    return (
      <div className="flex items-center justify-center h-80 gap-3 text-gray-400">
        <Loader2 size={24} className="animate-spin" />
        <span className="text-sm">Đang tải bảng công việc...</span>
      </div>
    );
  }

  // ── Error state ──────────────────────────────────────────────
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-80 gap-4 text-center">
        <div className="w-14 h-14 rounded-full bg-red-50 flex items-center justify-center">
          <AlertTriangle size={28} className="text-red-400" />
        </div>
        <div>
          <p className="text-sm font-medium text-gray-700 mb-1">Tải dữ liệu thất bại</p>
          <p className="text-xs text-gray-400">{error}</p>
        </div>
        <button
          onClick={fetchIssues}
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition"
        >
          <RefreshCcw size={14} />
          Thử lại
        </button>
      </div>
    );
  }

  // ── Board UI ─────────────────────────────────────────────────
  return (
    <div className="flex flex-col h-full">
      {/* Page title + meta */}
      <div className="mb-5">
        <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Board</h1>
        <p className="text-sm text-gray-500 mt-0.5">
          {issues.length} issue{issues.length !== 1 ? 's' : ''} · Cập nhật vừa xong
        </p>
      </div>

      {/* Divider */}
      <div className="border-b border-gray-200 mb-5" />

      {/* Kanban Grid — 4 cột */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 flex-1 min-h-0 pb-6">
        {COLUMN_STATUSES.map((status) => (
          <BoardColumn
            key={status}
            status={status}
            issues={grouped[status]}
          />
        ))}
      </div>
    </div>
  );
};

export default Board;
